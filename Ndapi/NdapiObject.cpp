...
namespace Ndapi
{
	...
	T NdapiObject::GetObjectProperty(int property_id)
	{
		...
		if (object == nullptr) {
			return safe_cast<T>(safe_cast<Object^>(nullptr));
		}
	...
	bool NdapiObject::HasOverriddenProperty(int property_id)
	{
		auto status = d2fobid_IspropDefault(NdapiContext::Context, _handler, property_id);
		CheckStatusAndThrow(status, "Error verifying if property was overidden.");
